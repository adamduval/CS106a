...
Complete - create test file for for check move down test case
Complete - Apply gravity
Complete - Loop through whole grid
...
def check_move(grid, x1, y1, x2, y2):
    ...
    False
    """
    # check to make sure move is not outside of grid
    move = check_grid(grid, x2, y2)
    if move is False:
        return False
    # check to make sure destination space is empty
    move = check_empty(grid, x2, y2)
    if move is False:
        return False
    # check to make sure down and left or down and right move is allowed
    move = check_dldr(grid, x1, y1, x2, y2)
    if move is False:
        return False
    return True
    pass
...
# checks move to make sure it is not outside of grid
def check_grid(grid, x2, y2):
    # check left is not off end of grid
    if x2 < 0:
        return False
        # check down is not end off of grid
    if y2 > (len(grid)-1):
        return False
        # check right is not  off end of grid
    if x2 > (len(grid[y2]) - 1):
        return False
    return True


# checks to make sure destination space at x2 y2 is empty
def check_empty(grid, x2, y2):
...
# checks to make sure down left or down right move is allowed.
def check_dldr(grid, x1, y1, x2, y2):
    # check down right
    if x2 - x1 > 0 and y2 - y1 > 0 and grid[y1][x1 + 1] is not None:
        return False
    # check down left
    if x2 - x1 < 0 and y2 - y1 > 0 and grid[y1][x1 - 1] is not None:
        return False
...
# applies various gravity effects (moves) to sand
def do_gravity(grid, x, y):
    ...
    [[None, None, None], [None, 's', None]]
    """
    # 1. Check if sand at x,y if not end
    if sand_present(grid, x, y) is not True:
        return grid
    # 2. Can it move down? Yes - do it
    if check_move(grid, x, y, x, y+1) is True:
        return move_down(grid, x, y)
    # 3. Can it move down-left? Yes - do it
    if check_move(grid, x, y, x-1, y+1) is True:
        return move_down_left(grid, x, y)
    # 4. Can it move down-right? Yes - do it
    if check_move(grid, x, y, x+1, y+1) is True:
        return move_down_right(grid, x, y)
    # 5. Sand can't move
    return grid
    pass


# check if sand is present to move
def sand_present(grid, x, y):
    if grid[y][x] == 's':
        return True
    return False
...
def move_down_right(grid, x, y):
    grid[y][x] = None
    grid[y+1][x+1] = 's'
    return grid


# applies brownian motion to sand
def do_brownian(grid, x, y, brownian):
    """
    Given grid, x,y, and brownian int 0..100.
    Do the random brownian move for that x,y.
    """
    # check if sand at x,y if not end
    if sand_present(grid, x, y) is not True:
        return grid
    num = random.randrange(100)
    if num < brownian:
        # coin = 0 move left, coin = 1 move right
        coin = random.randrange(2)
        if coin == 0:
            # check if move left legal
...
# Applies gravity and brownian to all sand
def do_whole_grid(grid, brownian):
    ...
    """
    # Call do gravity once for every x,y location in the grid and return grid, go bottom up
    # y value
